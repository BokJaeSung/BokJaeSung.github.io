...
문제는 Hugo 소스 없이 빌드 결과물만 저장소에 올리다 보니, CSS 파일이 바뀌어도 HTML의 해시값이 자동으로 갱신이 안 됐던 것. 임시로는 실제 CSS 파일의 해시를 직접 계산해서 HTML에 반영했음.
...
PaperMod 기본 레이아웃은 글 목록이 1열로 쭉 나열되는 형태였음. `assets/css/extended/custom.css`에 CSS Grid를 추가해서 2열로 바꿨음. 카드마다 높이를 `65vh`로 고정하고, 커버 이미지가 있으면 상단에 꽉 차게 표시되도록 했음.

```css
body:not(:has(.post-single)) .main {
  display: grid;
...
글 본문 페이지에서는 그리드가 적용되면 안 되니까 `:not(:has(.post-single))` 조건으로 목록 페이지에서만 동작하도록 스코프를 제한했음.

### 카드 호버 효과

카드에 마우스를 올리면 살짝 떠오르면서 그림자가 생기고, 커버 이미지가 약간 확대되도록 했음.
