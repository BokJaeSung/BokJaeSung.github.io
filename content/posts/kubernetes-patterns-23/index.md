...
즉 어찌어찌 탈출해 호스트 파일에 손을 뻗어도, **라벨이 안 맞아서** 커널이 거부한다. 컨테이너 탈출의 마지막 단계 — 호스트 파일 만지기 — 를 막는 최후의 벽이다. 호스트 OS가 SELinux를 지원해야 하며(RHEL 계열 기본, 우분투 계열은 유사 역할의 AppArmor), 두 모드가 있다.
